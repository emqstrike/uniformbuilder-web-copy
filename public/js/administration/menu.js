...
new Vue({
    ...
    computed: {
        filteredPages() {
            if (! this.search) {
                ...
                return this.pages.filter(page => page.page_name.toLowerCase().includes(this.search.toLowerCase()));
            }
        },
        page(page) {
            return 'here';
    ...
    methods: {
        add() {
            ...
            var lastMenuOrderId = this.menus[this.menus.length - 1].order_id;

            if (this.selectedPages.length === 0) {
                this.errors.push('Please select pages to add');
            } else {
                this.dialog = true;

                this.selectedPages.forEach((page) => {
                    menus.push({
                        route_name: page.code,
                        menu_text: page.page_name,
                        order_id: ++lastMenuOrderId,            
                        type: 'header'                          // default type
                    });
                });

                axios.post('create_menu_item', menus).then((response) => {
        ...
        cancel(menu) {
            Object.assign(menu, this.menuCache);
            this.toggleDialog();
        },
        checkAll() {
            ...
            if ((this.selectedPages.length > 0)) {
                ...
                for (var key in this.pages) {
                    let page = Object.assign({}, this.pages[key]);

                    Vue.delete(page, 'code');
                    Vue.delete(page, 'id');
                    Vue.delete(page, 'name');
                    Vue.delete(page, 'created_at');
                    Vue.delete(page, 'updated_at');

                    this.selectedPages.push(this.pages[key]);
                }
            }
        ...
        edit(menu) {
            this.menuCache = Object.assign({}, menu);
            this.menu = menu;

            var icon = this.icons.find(icon => icon.icon === menu.icon_class);

            if (icon) {
                this.iconValue = icon;
            } else {
                this.iconValue = this.icons[0];
            }
                    ...
                    if (menu.hasOwnProperty('menus')) {
                        if (menu.menus.length > 0) {
                            ...
                            menu.menus.forEach(function(_menu) {
                                _menu.parent_menu = "no parent";
                                _menu.parent_id = 0;
                                self.menus.splice(index, 0, _menu);
                            });
                        }
                    }
                } else {
        ...
        saveMenu() {
            ...
            axios.post('menu', this.menus).then((response) => {
                if (response.data.success == true) {
                    this.getMenuDataFromAPI();
                }
            });
        ...
        selectIcon(selectedOption, id) {
            this.menu.icon_class = selectedOption.icon;
        },
        toggleDialog() {
        ...
        updateOrderId() {
            ...
            self.menus.forEach(function(menu) {
                menu.order_id = parentMenuOrderCount++;
