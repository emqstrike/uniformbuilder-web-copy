...
AWS.config.update({accessKeyId:'AKIAJBGXZ7ET4HZD6YAA',secretAccessKey: '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'});
				...
				s3.putObject(params, function(err, data) {
				  if (err){
                    ...
                    var svgImage = document.querySelector('#jersey_front'),
                        ...
                        newImg.src = "data:image/svg+xml," + encodeURIComponent(svgStr);
...
function applyStyle(curr_style,index){
	...
	for(i=1;i<5;i++){
		...
		$('.base'+i)
			...
			.css('fill',curr_style['infbase'+i]);
			
		$('input.base'+i)
			.attr('value',curr_style['infbase'+i]);
	
		$('.cpicker .base'+i).siblings('.colorname').html(transColor(curr_style['infbase'+i]));
	...
	for(i=1;i<5;i++) {
		...
		// infusion - color values
		$('.infpicker input.infbase'+i).val( curr_style['infbase'+i] );
		console.log( $('.infpicker input.infbase'+i).val() );

		// infusion - color names
		$('.infpicker .infbase'+i).siblings('.colorname').html(transColor(curr_style['infbase'+i]));
	
	}

	$('#gradstop1').css( 'stop-color', curr_style['infbase1'] );
	$('#gradstop2').css( 'stop-color', curr_style['infbase2'] );
...
function applyStyleToThumb(curr_style,index){
	...
	for(i=1;i<5;i++){
		...
		currPiece.find('.infpicker .infbase'+i).siblings('.colorname').html(transColor(curr_style['infbase'+i]));
