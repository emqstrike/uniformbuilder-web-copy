...
bldr.panels = {
	...
	"back_jersey" 	: ['#path3293','#back_jersey_inner'],
	"back_panels"	: ['#path3285','#path3285'],
	"back_pant" 	: ['#g4453','#path32655','#path3273'],
...
bldr.pipings = {
	...
	"belt_loop" 		: ['#path3189','#path3272','#path3266'],
	...
	"back_pants" 		: ['#path3271','#path3200','#path3274'],
};
...
function refreshBackname() {
	...
	console.log('SAVING: refreshBackName');
	save();
}
...
function init(){
	...
	$('#collapseInf').on('shown.bs.collapse', function () {
		...
		$('.infusions svg').click(function () {
			...
			if (infusion == 'gradientInfusion' || infusion == 'circlesInfusion') {

				a = $('.infpicker')[2];
				a = $(a).find('.toggler, .colorname');
				$(a).css('opacity', '0');

				b = $('.infpicker')[3];
				b = $(b).find('.toggler, .colorname');
				$(b).css('opacity', '0');

				if (!infusion == 'gridInfusion') {

					c = $('.infpicker')[1];
					c = $(c).find('.toggler, .colorname');
					$(c).css('opacity', '0');
				...
				$('.infpicker .toggler, .infpicker .colorname').css('opacity', '1').show();
	...
	$('#collapseMat').on('shown.bs.collapse', function () {
		...
		$('.materials svg').click(function () {
			section = "materials";
			material = $(this).find('rect').css('fill');
			$('.picked').attr('class', '');
			$(this).attr('class', 'picked');
	...
	$('#eraser span').click(function () {
		...
		if (section == 'pipings' || document.target == 'pipings') {

			$('#path3087,#path3095').attr('class', 'arm_end tgt');
			$('#path3115').attr('class', 'back_piping tgt');
			$('#path3189,#path3272,#path3266').attr('class', 'belt_loop tgt');
			$('#path3136,#path3116').attr('class', 'center_piping tgt');
			$('#path3121,#path3138').attr('class', 'raglan tgt');
			$('#path3191').attr('class', 'side_pant tgt');
			$('#path3193, #path3287').attr('class', 'side_pants tgt');
			$('#path3159, #path3142').attr('class', 'side_panel_piping tgt');
			$('#path3163, #path3180').attr('class', 'sleeve_piping tgt');
			$('#path3271, #path3200,#path3274').attr('class', 'back_pants tgt');
		} else {
			...
			$('#selected #path3217, .option #path3217').attr('class', 'front_jersey tgt');
			$('#path4065, #path3935, #path5108,#path3545-2').attr('class', 'base_color tgt');
			$('#path3293, #back_jersey_inner').attr('class', 'back_jersey tgt');
			$('#path3285').attr('class', 'back_panels tgt');
			$('#path3317,#path3309').attr('class', 'bottom_sleeve tgt');
			$('#path3293, #path3301').attr('class', 'side_panel tgt');
			$('#path3325, #path3333').attr('class', 'sleeve_color tgt');
			$('#g4453, #path32655, #path3273').attr('class', 'back_pant tgt');
			$('#path4088, #path3221, #path321341,#path3213-4').attr('class', 'loops_tunnels tgt');
			$('#path41169, #path318921,#path3189-2').attr('class', 'pockets tgt');
			$('#path3341, #path334961').attr('class', 'back_knee tgt');
			$('#path3269, #path3273').attr('class', 'back_pant tgt');
			$('#path3181, #path31771').attr('class', 'bottom_pants tgt');

			$('#selected image, #sidebar image').attr('class', 'tgt');
	...
	$('#saveStyle').submit(function (event) {
		...
		$('#saveStyle button').before('<img src="assets/img/loading.gif">');
		
		$('#yourmodal button[type="submit"]').toggleClass('disabled');
...
function translate(p) {
	...
	t[11] = transColor(p.pipings.back_pants.stroke); //Back of Pant Piping
...
function highlightTargets() {
	...
	$("#selected .tgt").hover(
		function () {
		a = $(this).attr('class'); //DISPLAY OUTLINE
	...
	$('#selected ').on('click','path, image, tspan, text', function () {
		...
		if (typeof $(this).attr('class') != "undefined" && $(this).attr('class').indexOf('tgt') != -1) {
			...
			if (section == 'panels') { //IF IT IS A SOLID COLOR...
				...
				materialName = material.replace('url', '').replace('(', '').replace(')', '').replace(' ', '').replace('transparent', '').replace('"', '');
				
				materialName = materialName.split('#')[1].replace('"', '');
				if (myId == 'tspanmat' || myId == 'backnumbermat' || myId == 'backnamemat' || myId == 'tnleftmat' || myId == 'tnrightmat') { //MATERIAL ON NUMBERS / TEXT
					...
					Utils.fillMaterial($('#' + myId), materialName);
						...
						$.each($(targetClass), function () {
							...
							//$('#selected #' + thisId+', #sidebar #'+thisId).css('fill', "url(#"+materialName+")").css('fill-opacity', '1'); //STYLE WITH ID INFO
					...
					$.each($(targetClass), function () {
						...
						if (document.target == 'materials') {
							$('#selected ' + a + ', .option ' + a).css('fill', '#000').css('fill-opacity', '0'); // IT NEEDS TO HAVE A COLOR, OR THE PANEL IS UNSELECTABLE
...
function findPiece(id) {
	...
	} else if (id == 'path3193' || id == 'path3287') {
...
function htmlUpload() {
	...
	setTimeout(function () {
		...
		setTimeout(function () {
			...
			folder = 'newbalance/baseball/orders/' + orderId + '/';
			
			html = createHTML();
			
			notifyOrder(orderId, baseUrl+'/orders/' + orderId + '/' + orderId + '.html');
...
function notifyOrder(orderId, finalUrl) {
	...
	setTimeout(function(){
        var message={};
	       message.text = 'You have a new order (ID ' + orderId + '). <br/> <a href="' + finalUrl + '">Check order details.</a>';
           message.id = GetURLParameter('clientid');
...
function targetSVG() {
	...
	$('.tgt').attr('class', ''); //CLEAR TARGETS

	var paths = {

		'panels' : {
			'front_jersey' : ['#path3217'],
			'side_panels' : ['#path3293', '#path3301'],
			'bottom_sleeve' : ['#path3317', '#path3309'],
			'sleeve_color' : ['#path3325', '#path3333'],
			'base_color' : ['#path4065', '#path3935', '#path5108', '#path3545-2'],
			'back_jersey' : ['#path3281', '#back_jersey_inner'],
			'back_panels' : ['#path3285'],
			'back_pant' : ['#g4453', '#path32655', '#path3269', '#path3273'],
			'loops_tunnels' : ['#path4088', '#path3221', '#path321341', '#path3213-4'],
			'pockets' : ['#path41169', '#path318921', '#path3189-2'],
			'back_knee' : ['#path3341', '#path334961'],
			'back_pant' : ['#path3269', '#path3273']
	...
	for (i in paths) {
		...
		for (a in paths[i]) {
			...
			for (b in paths[i][a]) {
				...
				$('.option ' + paths[i][a][b]).css('fill', 'red').css('fill-opacity', '1');
...
function applyStyle(curr_style,index){
	...
	tempStyle = tempStyle
					.replace(/#d00000/g,'xb1'+base1)
					.replace(/#ffffff/g,'xb2'+base2)
	...
	if(curr_style.panels.sleeve_color.infusion != ''){
		...
		$('#selected').find('#path3333inf, #path3325inf').attr("style","fill:url('#camoInfusion'); stroke-width:0;;opacity:1");
...
function createQsThumbs(){
	
	console.log('createQsThumbs 1!');

	if(q >= 3){
		
		setTimeout(function(){
			console.log('createQsThumbs 2!');
			
			$('#jsonpieces svg').each(function(){
			
				console.log('createQsThumbs 3!');
			
				var index = $(this).index();
			
				applyStyleToThumb(styles[index],index);
